<section>
  ...
  <form method="post" action="edit.php?id=<?= $_GET["id"] ?>">
    <div class="form-group">
      ...
      <input type="text" class="form-control" name="title" aria-describedby="nameHelp" id="title" placeholder="Ingrese..." value="<?= isset($_POST["title"]) ? $_POST["title"] : $serieObtained->title ?>">
    </div>
    <div class="form-group">
      ...
      <select class="form-select" id="platformsSelect" name="platform">
        ...
        <?php foreach ($platformList as $platform) { ?>
          <option value="<?= $platform->id ?>" <?= isset($_POST["platform"]) ? ($_POST["platform"] == $platform->id ? "selected" : "") : ($serieObtained->platform_id == $platform->id ? "selected" : "") ?>><?= $platform->name ?></option>
        <?php } ?>
      </select>
    </div>
    <div class="form-group">
      ...
      <select class="form-select" id="directorsSelect" name="director">
        ...
        <?php foreach ($directorList as $director) { ?>
          <option value="<?= $director->id ?>" <?= isset($_POST["director"]) ? ($_POST["director"] == $director->id ? "selected" : "") : ($serieObtained->director_id == $director->id ? "selected" : "") ?>><?= $director->name ?> <?= $director->last_name ?></option>
        <?php } ?>
      </select>
    </div>
    <div class="form-group">
      ...
      <div class="row px-2">
        <?php foreach ($actorList as $actor) { ?>
          <div class="form-check form-switch col-12 col-sm-12 col-md-6 col-lg-4">
            <input class="form-check-input" type="checkbox" id="actor_check_<?= $actor->id ?>" name="actor[]" value="<?= $actor->id ?>" <?= gettype(array_search($actor->id, $actorObtained)) === "integer" ? "checked" : "" ?>>
    ...
    <div class="form-group">
      ...
      <div class="row px-2">
        <?php foreach ($languageList as $audio) { ?>
          <div class="form-check form-switch col-12 col-sm-12 col-md-6 col-lg-4">
            <input class="form-check-input" type="checkbox" id="audio_check_<?= $audio->id ?>" name="audio[]" value="<?= $audio->id ?>" <?= gettype(array_search($audio->id, $languagesObtained->audios)) === "integer" ? "checked" : "" ?>>
    ...
    <div class="form-group">
      ...
      <div class="row px-2">
        <?php foreach ($languageList as $subtitle) {  ?>
          <div class="form-check form-switch col-12 col-sm-12 col-md-6 col-lg-4">
            <input class="form-check-input" type="checkbox" id="subtitle_check_<?= $subtitle->id ?>" name="subtitle[]" value="<?= $subtitle->id ?>" <?= gettype(array_search($subtitle->id, $languagesObtained->subtitles)) === "integer" ? "checked" : "" ?>>
